...
export async function GET(_request: Request, { params }: StatusRouteProps) {
  const webhookUrl = process.env.N8N_MP3_STATUS_WEBHOOK_URL;

  if (!webhookUrl) {
    return jsonError(
      "Falta configurar N8N_MP3_STATUS_WEBHOOK_URL en el servidor.",
      500
    );
  }
  ...
  try {
    ...
    if (!n8nResponse.ok || !payload) {
      return jsonError(
        "No se ha podido consultar el estado del MP3.",
